...
fn clasify(hand: &Hand) -> WinType {
    ...
    let mut nr_wildcards = 0;

    for card_id in hand.cards {
        if card_id == WILDCARD_VALUE {
            nr_wildcards += 1;
            continue;
        }
    ...
    let highest_count = match counts.last() {
        Some(last_count) => last_count.amount + nr_wildcards,
        None => nr_wildcards, // only wildcards, so 5 of a kind
    };
    ...
    if highest_count == 5 {
        ...
        if counts[counts.len() - 2].amount == 2 || nr_wildcards > 1 {
            WinType::TwoPair
        } else {
            WinType::OnePair
        }
